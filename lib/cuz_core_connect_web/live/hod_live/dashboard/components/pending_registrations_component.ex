defmodule CuzCoreConnectWeb.HODLive.Dashboard.PendingRegistrationsComponent do
  ...
  def handle_event("approve", %{"id" => id}, socket) do
    ...
    case Registrations.update_registration(reg, %{hod_status: "APPROVED"}) do
      {:ok, _} ->
        {:noreply, socket |> put_flash(:info, "Approved.") |> assign(:registrations, Registrations.list_pending_for_hod())}
      {:error, _} ->
  ...
  def handle_event("reject", %{"id" => id}, socket) do
    ...
    case Registrations.update_registration(reg, %{hod_status: "REJECTED"}) do
      {:ok, _} ->
        {:noreply, socket |> put_flash(:info, "Rejected.") |> assign(:registrations, Registrations.list_pending_for_hod())}
      {:error, _} ->
        {:noreply, put_flash(socket, :error, "Failed.")}
    end
  end

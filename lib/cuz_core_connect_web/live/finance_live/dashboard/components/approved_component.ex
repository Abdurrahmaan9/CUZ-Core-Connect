defmodule CuzCoreConnectWeb.FinanceLive.Dashboard.ApprovedComponent do
  ...
  def update(assigns, socket) do
    registrations = Registrations.list_by_payment_status("APPROVED")
  ...
  def render(assigns) do
    ~H"""
    <div>
      <h2 class="text-lg font-semibold mb-4">Verified Payments</h2>
      <%= if Enum.empty?(@registrations) do %>
        <div class="border-2 border-dashed border-base-300 rounded-xl p-12 text-center">
          <p class="text-base-content/50">No verified payments yet.</p>
        </div>
      <% else %>
          ...
          <table class="table w-full">
            ...
            <tbody>
              <%= for reg <- @registrations do %>
                <tr id={"finance-approved-#{reg.id}"} class="hover:bg-base-200/30">
                  <td>
      ...
      <%= if @selected do %>
        <.live_component
          ...
          id="finance-approved-details-modal"
          registration={@selected}
          parent_module={__MODULE__}
          parent_id="finance-approved"
        />
      <% end %>
    </div>
    """

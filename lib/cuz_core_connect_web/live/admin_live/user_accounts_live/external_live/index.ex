defmodule CuzCoreConnectWeb.Admin.UserAccounts.External do
  ...
  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> assign(
       page_title: "External Users",
       current_page: :external_users,
       show_form: false,
       form: nil,
  ...
  defp apply_action(socket, :new, _params) do
    form =
      %User{is_active: true, status: "ACTIVE", user_role: "student"}
      |> User.admin_changeset(%{})
      |> to_form()

    socket
    |> assign(page_title: "New External User", show_form: true, form: form, access_user: nil)
  end
  ...
  defp apply_action(socket, :edit, %{"id" => id}) do
    ...
    if user.user_role != "student" do
      socket
      |> put_flash(:error, "That account is an internal staff user.")
      |> push_navigate(to: ~p"/admin/user-accounts/external")
    else
      form = user |> User.admin_changeset(%{}) |> to_form()

      socket
      |> assign(page_title: "Edit External User", show_form: true, form: form, access_user: nil)
    end
  end
  ...
  def handle_event("new", _params, socket) do
    {:noreply, push_patch(socket, to: ~p"/admin/user-accounts/external/new")}
  end
  ...
  def handle_event("close", _params, socket) do
    {:noreply,
     ...
     |> push_patch(to: ~p"/admin/user-accounts/external")}
  end

  def handle_event("validate", %{"user" => params}, socket) do
    params = Map.put(params, "user_role", "student")
  ...
  defp load_users(socket) do
    ...
    all_users = Accounts.list_all_external_users()
    ...
    >
      <div class="space-y-6">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 class="text-2xl font-bold text-base-content">External Users</h1>
            <p class="text-sm text-base-content/60">
              Student accounts that register and track programmes online.
            </p>
          </div>
        ...
        <div class="rounded-box border border-base-300 bg-base-100 shadow-sm">
          <div class="flex flex-col gap-3 border-b border-base-300 p-4 sm:flex-row sm:items-center sm:justify-between">
            <h2 class="font-semibold text-base-content">All external users</h2>
            <form phx-change="search" id="external-user-search" class="w-full sm:w-72">
              <input
                type="search"
                name="query"
                value={@query}
          ...
          <div class="overflow-x-auto">
            <table class="table">
              ...
              <tbody>
                <tr :for={user <- @users} id={"external-user-#{user.id}"} class="hover">
                  <td>
                    <div class="flex items-center gap-3">
                      <div class="flex size-10 items-center justify-center rounded-full bg-secondary/10 text-sm font-semibold text-secondary">
                  ...
                  <td>
                    <span class="badge badge-sm badge-secondary capitalize">{user.user_role}</span>
                  </td>
                  <td>
                    <span class={[
                      "badge badge-sm",
      ...
      >
        <div class="w-full max-w-lg rounded-2xl bg-base-100 p-6 shadow-2xl" phx-click="noop">
          <h3 class="mb-1 text-lg font-bold">
            {if @form.data.id, do: "Edit external user", else: "Create external user"}
          </h3>
          <p class="mb-4 text-sm text-base-content/60">
            {if @form.data.id,
              do: "Update student account details.",
              else: "Creates a student account. Password is generated automatically."}
          </p>
          ...
          >
            ...
            >
              ...
              Role is fixed to <strong>student</strong>. A secure password is generated and emailed
              here. In development, open

  ...
  def render(assigns) do
    ...
    <div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <.stat_card label="Approved Today" value={@stats.approved_today} color="success" icon="hero-check-circle" />
        <.stat_card label="Rejected" value={@stats.rejected} color="error" icon="hero-x-circle" />
        <.stat_card label="Total Reviewed" value={@stats.total_reviewed} color="info" icon="hero-academic-cap" />
      </div>

      <div class="bg-base-200/40 rounded-xl p-6">
        <h3 class="font-medium mb-4 flex items-center gap-2">
          <.icon name="hero-clipboard-document-list" class="h-5 w-5 text-primary" />
          Quick Actions
        </h3>
